...
T _liveDataTransform<T>(Observable<T> data) {
  return data.value;
}
...
class _NotifierData<T, B extends ChangeNotifier> extends Observable<T> {
  final B base;
...
class _LiveDataMirror<T> extends _NotifierData<T, Observable<T>> {
  _LiveDataMirror(Observable<T> base, {T Function(Observable<T>)? transform})
    : super(
        base,
...
class _TransformedLiveDataMirror<T, S, B extends Observable<S>>
...
class _StreamData<T, D, S extends Stream<D>> extends Observable<T?> {
  final S base;
