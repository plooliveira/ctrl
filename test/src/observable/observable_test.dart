...
class NullableInitialLiveData<T> extends Observable<T?> {
  T? _value;
  @override
  T? get value => _value;
...
class TestMutableLiveData<T> extends MutableObservable<T> {
...
void main() {
  group('LiveData Core Functionality', () {
    test(
      ...
      () {
        final liveData = MutableObservable(10);
        int? receivedValue;
        int callCount = 0;
  ...
  group('MutableLiveData Functionality', () {
    ...
    test(
      ...
      () {
        final liveData = MutableObservable(10, true);
        int callCount = 0;
        liveData.subscribe((value) => callCount++);
    ...
    test('update should always notify listeners', () {
      final liveData = MutableObservable([1, 2, 3]);
      int callCount = 0;
      liveData.subscribe((value) => callCount++);
    ...
    test('immutable getter should return the same instance as LiveData', () {
      final mutable = MutableObservable(10);
      final immutable = mutable.immutable;

      expect(immutable, same(mutable));
      expect(immutable, isA<Observable<int>>());
  ...
  group('_defaultChangeDetector Behavior (tested via MutableLiveData)', () {
    ...
    test(
      ...
      () {
        final list = [1, 2, 3];
        final liveData = MutableObservable(list);
        int callCount = 0;
        liveData.subscribe((_) => callCount++);
    ...
    test('should notify when assigning a NEW object instance (Map)', () {
      final liveData = MutableObservable({'a': 1});
      int callCount = 0;
      liveData.subscribe((_) => callCount++);
  ...
  group('LiveData Extensions', () {
    group('ListLiveData', () {
      ...
      test('filtered should create a LiveData that filters items', () {
        final source = MutableObservable([1, 2, 3, 4, 5]);
      ...
      test('notNull should create a LiveData that filters out nulls', () {
        final source = MutableObservable<List<int?>>([1, null, 3, null, 5]);
    ...
    group('Mirror', () {
      test('should mirror value from source', () {
        final source = MutableObservable(10);
  ...
  group('LiveData Scope Management', () {
    test(
      ...
      () {
        ...
        final source = MutableObservable(10, false, scope);
  ...
  group('LiveData Factory Methods', () {
    group('fromValueNotifier', () {
      test('should create LiveData from ValueNotifier', () {
        ...
        final liveData = Observable.fromValueNotifier(notifier);
      ...
      test('should be disposed when registered in scope', () {
        final scope = DataScope();
        final notifier = ValueNotifier<int>(10);
        final liveData = Observable.fromValueNotifier(notifier, scope);
      ...
      test('should work with different types', () {
        ...
        final stringData = Observable.fromValueNotifier(stringNotifier);
    ...
    group('fromStream', () {
      test('should create LiveData from Stream with initial value', () {
        ...
        final liveData = Observable.fromStream(controller.stream, 0);
      ...
      test('should be disposed when registered in scope', () async {
        final scope = DataScope();
        final controller = StreamController<int>();
        final liveData = Observable.fromStream(controller.stream, 0, scope);
      ...
      test('should work with different types', () async {
        ...
        final liveData = Observable.fromStream(controller.stream, 'initial');
  ...
  group('HotswapLiveData', () {
    group('Creation and Initialization', () {
      test('should create HotswapLiveData from a base LiveData', () {
        final base = MutableObservable<int>(10);
    ...
    group('Hotswap Functionality', () {
      test('should hotswap to new base LiveData and update value', () {
        final base1 = MutableObservable<int>(10);
        final base2 = MutableObservable<int>(20);
    ...
    group('Advanced Scenarios', () {
      test('should handle multiple sequential hotswaps correctly', () {
        final base1 = MutableObservable<int>(10);
        final base2 = MutableObservable<int>(20);
        final base3 = MutableObservable<int>(30);
        final base4 = MutableObservable<int>(40);
        final hotswap = base1.hotswappable();
      ...
      test('should work with different base implementations of same type', () {
        final mutable1 = MutableObservable<int>(10);
        final hotswap = mutable1.hotswappable();

        expect(hotswap.value, 10);

        final mutable2 = MutableObservable<int>(20);
        hotswap.hotswap(mutable2);
        expect(hotswap.value, 20);

        final notifier = ValueNotifier<int>(30);
        final fromNotifier = Observable.fromValueNotifier(notifier);
